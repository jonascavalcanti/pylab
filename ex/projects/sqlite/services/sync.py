...
class Sync():
    ...
    #     """
    #     gws_groups = google.Members().list_sub_groups(sync_group, email_suffixes_allow=Const.EMAIL_SUFFIX)
    ...
    def __sync_gws_to_gh(self):
        ...
        if result.has_errors():
            raise Exception(f"{self.__client.logkey}sync groups failed")
    ...
    def __sync_from_google_work_space(self, group):
        try:
            ...
            if gh_group is None:
                gh_group_out_of_root = self.__github_groups.get(group.name)
